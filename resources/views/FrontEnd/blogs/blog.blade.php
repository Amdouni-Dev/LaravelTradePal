...
<div id="main">
    <div id="animation-banner" class="web">
        ...
        <div class="content">
            <article class="testify" style="background-color:#bddde3">
                <div class="container top-sep" style="background-color: #bddde3; margin:0">
                    <div class="thanks">
                        ...
                        <br><br>
                        <div><i style="font-size:1.5rem;color:#bfd578" class="bx bxs-purchase-tag-alt me-1"></i>
                        ...
                        <div>
                            <div class="third">
                                <div>
                                    @auth
                                        <form
                                            action="{{ route('like.toggle', ['user_id' => Auth::user()->id, 'blog_id' => $blog->id]) }}"
                                            method="post">
                                            @csrf
                                            ...
                                            @else
                                                <button type="submit" class="like-button">
                                                    <i style="font-size: 3rem; color: #a54458"
                            ...
                            <div class="third">
                                <div>
                                    <i style="font-size:3rem;color:#3c7aa9"
                                        class="bx bxs-share-alt me-1 btn facebook"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                <td align="right">
                                                    @auth
                                                        @if (auth()->user()->username == $comment->user->username)
                                                            <form action="{{ route('comments.delete', ['comment_id' => $comment->id]) }}"
                                                                method="POST"
                                    ...
                                    <tr>
                                        <td style="min-width: 45rem;">
                                            <p class="lead emoji-picker-container" style="background:white">
                                                <textarea name="content" style="min-height:0rem" class="form-control textarea-control" rows="3"
                                                    placeholder="Ecrire votre commentaire" data-emojiable="true"></textarea>
                                            </p>
    ...
    <script>
        ...
        twitter.addEventListener('click', function(ev) {
            let shareUrl =
                `https://twitter.com/intent/tweet?url=${url}&text=${text}&via=${user_id}&hashtags=${hash_tags}`;
            window.open(shareUrl, "NewWindow", params);
        });
    </script>
